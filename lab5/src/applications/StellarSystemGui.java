...
public class StellarSystemGui {
  ...
  private void changeStellarSystem(int flag)
      throws FileChooseException, IOException {
    if (flag == 1) {
      stellarSystemFile =
          new File("src/Spring2019_HITCS_SC_Lab5-master/StellarSystem.txt");
    } else if (flag == 2) {
      stellarSystemFile = new File(
          "src/Spring2019_HITCS_SC_Lab3-master/StellarSystem_Medium.txt");
    } else if (flag == 3) {
      stellarSystemFile = new File(
          "src/Spring2019_HITCS_SC_Lab3-master/StellarSystem_Larger.txt");
    } else {
      System.out.println("文件选取参数有误！");
    }
    stellarSystem = new StellarSystem();
    stellarSystem.setReadFile(stellarSystemFile);
      ...
      @Override public void paint(Graphics g) {
        ...
        for (int i = count - 1; i >= 0; i--) {
          ...
          StellarSystemObject planet =
              stellarSystem.getTrack(i + 1).getIndexPhysicalObject(1);
          ...
          g2d.draw(new Ellipse2D.Double(
              x + Math.cos(angle + (controlSpan % 2) * time * angleDif)
                  * (d + 5) - 5,
              y + Math.sin(angle + (controlSpan % 2) * angleDif * time)
                  * (d + 5) - 5,
              10, 10));
        }
      }
  ...
  private void initFunc2() {
    ...
    JTextField textField =
        new JTextField("按顺序输入：轨道半径、名称、形态、颜色、半径、公转速度、公转方向、起始角度");
    panel2.add(textField);
    button2.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        try {
          ...
          if (stellarSystem
              .getTrackByRadius(Double.parseDouble(strings[0])) == null) {
            Track<StellarSystemObject> track =
                new Track<>(Double.parseDouble(strings[0]));
            StellarSystemObject planet = new StellarSystemObject(strings[1],
                strings[2], strings[3], Double.parseDouble(strings[4]),
                Double.parseDouble(strings[5]), strings[6],
      ...
      public void actionPerformed(ActionEvent e) {
        try {
          ...
          if (Integer.parseInt(strings[0]) <= stellarSystem.getTracksNumber()) {
            ...
            targetTime.set(Integer.parseInt(strings[1]),
            ...
            if (targetTime.before(stellarSystem.getReadTime())) {
              ...
              String targetTimeString =
                  strings[1] + "年" + strings[2] + "月" + strings[3] + "日"
                      + strings[4] + "时" + strings[5] + "分" + strings[6] + "秒";
              textField5
                  .setText(
                      "物体" + strings[0] + "在指定时刻" + targetTimeString + "的角度位置是"
                          + stellarSystem.calculatePosition(stellarSystem
                              .getTrack(Integer.parseInt(strings[0]))
                              .getIndexPhysicalObject(1), targetTime)
                          + "°");
            }
    ...
    button6.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        try {
          ...
          if (num1 > stellarSystem.getTracksNumber() || num1 < 0
              || num2 > stellarSystem.getTracksNumber() || num2 < 0) {
            ...
            if (num1 == num2) {
              ...
              if (num1 == 0) {
                textField6.setText("恒星" + "和行星" + num2 + "的距离是 "
                    + stellarSystem.getPhysicalDistanceStar(
                        stellarSystem.getTrack(num2).getIndexPhysicalObject(1))
                    + " KM");
              } else if (num2 == 0) {
                textField6.setText("行星" + num1 + "和恒星的距离是 "
                    + stellarSystem.getPhysicalDistanceStar(
                        stellarSystem.getTrack(num1).getIndexPhysicalObject(1))
                    + " KM");
              } else {
    ...
    button9.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        try {
          ...
          logger.info("成功将系统信息写入到默认文件:src/outputFile/StellarSystem.txt");
